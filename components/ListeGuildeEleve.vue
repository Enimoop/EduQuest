  ...
  <div class="container mt-5">
    <div class="list-details">
      ...
      <div v-if="guildes.length === 0" class="text-center">
        <p>Vous ne faites partie d'aucune guilde</p>
      </div>
      <div v-else>
        <div class="row">
...
const fetchGuildes = async (page: number, pageSize: number) => {
  try {
    const response = await axios.get(`http://localhost:3001/guildes/eleve/${id}`, {
      params: {
        page,
        pageSize
      }
...
const paginatedGuildes = computed(() => {
  const start = (currentPage.value - 1) * pageSize;
  const end = start + pageSize;
  return guildes.value.slice(start, end);
});
